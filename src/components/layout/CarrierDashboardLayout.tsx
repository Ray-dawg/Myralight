...
interface CarrierDashboardLayoutProps {
  children: ReactNode;
}

export default function CarrierDashboardLayout({
  children,
}: CarrierDashboardLayoutProps) {
  const { logout } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();

  const navItems = [
    { icon: Truck, label: "Fleet", path: "/carrier/fleet" },
    { icon: Users, label: "Drivers", path: "/carrier/drivers" },
    { icon: FileText, label: "Documents", path: "/carrier/documents" },
    { icon: BarChart3, label: "Analytics", path: "/carrier/analytics" },
    { icon: AlertCircle, label: "Claims", path: "/carrier/claims" },
    { icon: MessageCircle, label: "Messages", path: "/carrier/messages" },
    { icon: Settings, label: "Settings", path: "/carrier/settings" },
  ];
                ...
                <div className="relative">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      ...
                      >
                        <img
                          src="https://api.dicebear.com/7.x/avataaars/svg?seed=carrier"
                          alt="Profile"
                    ...
                    <DropdownMenuContent align="end" className="w-56">
                      ...
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                        onClick={() => navigate("/carrier/settings")}
                      >
                      ...
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                        onClick={() => navigate("/carrier/help")}
                      >
                        <HelpCircle className="mr-2 h-4 w-4" />
                        <span>Help & Support</span>
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onClick={() => navigate("/carrier/docs")}
                      >
